...
def make_samples(data_path, kb_path, limit=None):
    ...
    with open(data_path) as f_data, open(kb_path) as f_kb:
        for line_data, line_kb in tqdm(list(zip(f_data, f_kb))):
            line_idx += 1
            valid_data_dir = './../../data/data_analysis_valid/'
            invalid_data_dir = './../../data/data_analysis_invalid/'
            #original_example_path = '{:08d}_original.json'.format(line_idx)
            #processed_example_path = '{:08d}_proc01.json'.format(line_idx)
            event_example_path = '{:08d}_proc02.json'.format(line_idx)
            #kbsample_path = '{:08d}_kb.json'.format(line_idx)
            example = json.loads(line_data)
            kb = json.loads(line_kb)

            if is_sample_valid(example, kb):
                dir_path = valid_data_dir
            else:
                dir_path = invalid_data_dir
            #write_sample_file(example, dir_path+original_example_path)
            #write_sample_file(kb, dir_path+kbsample_path)
            ...
            #write_sample_file(example, dir_path+processed_example_path)
            event_data = factor_into_events(line_idx, example)
            write_sample_file(event_data, dir_path+event_example_path)
