...
class AgentGPT2(GPT2LMBase):
    def __init__(self, 
                 gpt2_type: str = "gpt2", 
                 device: Union[torch.device, str] = "cuda", 
                 max_length: Optional[int] = None):
        super(AgentGPT2, self).__init__(gpt2_type=gpt2_type, 
                                           device=device, 
...
class AgentGPT2Evaluator(Evaluator):
    def __init__(self, max_generation_len=None, 
                 temp=1.0, top_k=None, top_p=None,
                 verbose=True) -> None:
        super(AgentGPT2Evaluator, self).__init__()
    ...
    def evaluate(self, model: AgentGPT2, scenes: List[Scene]) -> Optional[Dict[str, Any]]:
        if self.verbose:
